...
async def sample(model, *, var_names=None, asset_url="https://pymc-labs.github.io/nuts-rs-wasm/nuts/", **options):
    ...
    run = pyjs.js.Function("base", "config", "options", "view", "refresh", """
      return (async () => {
    ...
    with pyjs.callable_context(memory_view) as refresh:
        output = json.loads(str(await run(asset_url, json.dumps(compiled.config()),
                                         json.dumps(options), memory_view(), refresh)))
    result = load_worker_result(output['manifest'])
    idata = to_inference_data(result)
    idata.attrs.update(compile_seconds=compile_seconds, sampling_seconds=output['seconds'])
    return idata
